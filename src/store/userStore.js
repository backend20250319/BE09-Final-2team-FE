    ...
    (set, get) => ({
      ...
      tempLogin: async (loginId, password) => {
        try {
          ...
          try {
            const userId = user.id || response.data.data.id;
            await websocketManager.connect(userId, user);
      ...
      checkAuthStatus: async () => {
        try {
          ...
          if (response.data.success && response.data.data && response.data.data.id) {
            ...
            // 인증 확인 성공 시 WebSocket 연결
            try {
              const userId = userData.id;
              await websocketManager.connect(userId, userData);
              console.log("✅ 인증 확인 후 WebSocket 연결 성공");
            } catch (error) {
              console.error("❌ 인증 확인 후 WebSocket 연결 실패:", error);
            }
